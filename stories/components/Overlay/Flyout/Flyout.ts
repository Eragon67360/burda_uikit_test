...
export const createFlyout = ({ items, label, icon = 'gift' }: FlyoutArgs) => {
  let isOpen = false;
  ...
  const defaultClasses =
    'bg-base-black group-hover:bg-secondary-dark text-base-white group-hover:w-[3.625rem] transition-all ease-in-out duration-300';
  const openClasses = 'bg-secondary-interaction text-base-black';
  const defaultButtonContainerClasses =
    'text-base-black hover:text-secondary-dark transition-all ease-in-out duration-300 transform-gpu hover:w-[3.625rem] z-10';
  ...
  const baseMobileContainerClasses =
    'block sm:hidden fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-32px)] mx-auto rounded-2xl flex flex-col items-center justify-end overflow-hidden transition-opacity duration-500 pointer-events-none z-50';
  ...
  const handleClasses =
    'absolute top-[0.38rem] left-1/2 -translate-x-1/2 w-[3.9375rem] h-[0.125rem] bg-neutral-400 z-50 transition-all duration-500';
